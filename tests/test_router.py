...
class TestRouterMethods(unittest.TestCase):
    ...
    def test_routing(self):
        ...
        params = {
            ...
            'ROUTER_HDGS_SEGMENTS': 20,
            'ROUTER_DELTA_TIME_HRS': 1}

        router.recursive_routing(iso, boat, winds, 3600, params, verbose=False)
        return None
    ...
    def test_pruning(self):
        ...
        actual = {
            'azi1': np.array([124.984596, 314.67391, 140.422624, 320.6739084]),
            's12': np.array([14081.4889, 22591.7854, 13866.1776, 19939.24846]),
            'lats': np.array([39.92720749, 20.14341011, 29.903555, 20.139284]),
            'lons': np.array([40.13496081, 19.84634009, 30.091473, 19.879143]),
            't12': 3600}

        bins = [120, 135, 140, 250, 300, 320, 340]
        calc = router.prune_isochrone(iso, 'azi1', 's12', bins, True)

        for k in actual.keys():
            self.assertTrue(np.allclose(actual[k], calc[k]))
        return None
